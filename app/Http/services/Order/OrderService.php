...
class OrderService
{
    ...
    {
        $order = Cart::selectRaw('c.id as customer_id, c.name as customer_name, c.address as customer_address, c.phone as customer_phone, c.email as customer_email, carts.created_at, sum(price*quantity) as total, carts.status')
            ->join('customers as c', 'c.id', '=', 'carts.customer_id')
